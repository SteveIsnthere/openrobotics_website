...
const Recruitment = () =>{
  const positions:Array<Role> = [
    new Role("Full Stack Developer", 2, "yuh fajkd adjldsf afkjdsfs adfkjsfkasdjfa;slkfj ds f;sdkfj sd;kljf","Admin"),
  ...
  return (
    <section className='flex flex-col m-[10%] py-10 md:mt-24 sm:mt-60 sm:max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg justify-center items-center'>
         ...
         {positions.map((position:Role) => (
           <Position name={position.title} positions={position.positions} team={position.team} desc={position.description}/>
       ))}
